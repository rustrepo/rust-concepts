...
// Implementing fmt::Display trait for Category to format the output as a string
    ...
    //The generic Item<T> struct has a T type parameter. By requiring T to implement fmt::Display in impl<T: fmt::Display>, you ensure that the price field of an Item can also be formatted as a string.
    ...
    fn main(){
        ...
        let mut inventory: Vec<Item<f64>> = vec! [
         ...
         When to Use Which
        Use if let:
        When you want to match a specific pattern (e.g., an enum variant, destructuring a tuple, etc.).
        Use ==:
        When you simply want to compare values for equality.
        */
